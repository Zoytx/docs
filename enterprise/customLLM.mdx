...
malakai will fine-tune a custom model for your enterprise using transcripts from succesful prior calls. Then malakai will host that LLM and provided dedicated infrastrucure to enable phone conversations with sub-second latency.

malakai will also enable you to connect to a custom LLM & will host that LLM to drive latency down further.
...
## How do I fine-tune with malakai?

To inquire about fine-tuning connect with the malakai team [here](https://www.malakai.ai/survey/).
...
The malakai team will advise on connection method, requirements for the connection, etc.
